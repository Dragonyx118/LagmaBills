...
void IRAM_ATTR motorControl(int pwmPin, int in1, int in2, int speed) {
  speed = constrain(speed, -255, 255);
  if (speed > 0)      { digitalWrite(in1, HIGH); digitalWrite(in2, LOW);  ledcWrite(pwmPin,  speed); }
  else if (speed < 0) { digitalWrite(in1, LOW);  digitalWrite(in2, HIGH); ledcWrite(pwmPin, -speed); }
  else                { digitalWrite(in1, LOW);  digitalWrite(in2, LOW);  ledcWrite(pwmPin,  0);     }
}

void IRAM_ATTR setMotori(int fl, int fr, int rl, int rr) {
  velFL = fl; velFR = fr; velRL = rl; velRR = rr;
  motorControl(FL_PWM, FL_IN1, FL_IN2, fl);
  motorControl(FR_PWM, FR_IN1, FR_IN2, fr);
  motorControl(RL_PWM, RL_IN1, RL_IN2, rl);
  motorControl(RR_PWM, RR_IN1, RR_IN2, rr);
}
...
void eseguiCmd(const MotorCmd& c) {
  switch (c.type) {
    ...
    case CMD_MOTOR_SINGLE: {
      ...
      int speed  = c.val[1];
      switch (motore) {
        case 0: motorControl(FL_PWM,FL_IN1,FL_IN2,speed); velFL=speed; break;
        case 1: motorControl(FR_PWM,FR_IN1,FR_IN2,speed); velFR=speed; break;
        case 2: motorControl(RL_PWM,RL_IN1,RL_IN2,speed); velRL=speed; break;
        case 3: motorControl(RR_PWM,RR_IN1,RR_IN2,speed); velRR=speed; break;
      }
...
void setup() {
  ...
  ledcAttach(FL_PWM, PWM_FREQ, PWM_RES);
  ledcAttach(FR_PWM, PWM_FREQ, PWM_RES);
  ledcAttach(RL_PWM, PWM_FREQ, PWM_RES);
