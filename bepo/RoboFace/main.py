...
class RoboEyes:
    ...
    def _run_sequence(self, ct):
        ...
        if seq==SEQ_SLEEP:
            ...
            d=self.seq_data
            sym_chars=list(range(7))  # 0=cherries,1=seven,2=diamond,3=bell,4=lemon,5=grapes,6=watermelon
            if 'font' not in d:
            ...
            if elapsed<6.5:
                ...
                for ri in range(3):
                    ...
                    # simboli sopra/sotto (più piccoli)
    ...
    def drawEyes(self):
        ...
        if self.focused:
            # sopracciglia dritte e ravvicinate per concentrazione
        ...
        if self.proud:
            ...
            # highlight lucido nell'angolo superiore degli occhi
        ...
        if self.nervous:
            ...
            # micro-tremito orizzontale rapido
            jitter=int(math.sin(self.nervous_phase*7)*5)
            self.eyeLx+=jitter;self.eyeRx+=jitter
            # ogni ~1.5s guarda di lato di scatto
            period=int(self.nervous_phase/(math.pi))*1
    ...
    def _start_listener(self):
        ...
        def listen():
            try:
                ...
                def idle_mood_list_is_seq(r):
                    # True se l'ultimo mood avviato era una sequenza ed è già finita
                    return r.sequence is None
                while True:
...
def main():
    ...
    # --- IDLE: cambiano automaticamente in loop ---
    # Emozioni idle "evento" (esclude DEFAULT e sequenze pesanti se vuoi)
    idle_events = [
    ...
    # Stato macchina idle: 'neutral' | 'event'
    idle_state = ['neutral']          # neutral = DEFAULT, event = mood casuale
    idle_timer = [pygame.time.get_ticks()]
    # Quanto stare in neutral prima del prossimo evento (8-18 secondi)
    neutral_duration = [random.randint(8000, 18000)]
    # True se un comando esterno ha interrotto l'idle
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # --- tasti comando (interrompono l'idle) ---
        ...
        # --- logica idle automatica ---
        if idle_state[0] == 'neutral':
            # aspetta neutral_duration, poi lancia evento casuale
            if ct - idle_timer[0] >= neutral_duration[0]:
                ...
                idle_state[0] = 'event'
                idle_timer[0] = ct
                neutral_duration[0] = dur  # riusato come event_duration

        elif idle_state[0] == 'event':
            seq_done = robo.sequence is None
        ...
        robo.update()
        # se è arrivato un comando via socket, resetta l'idle verso neutral dopo la durata
        if robo.next_mood is not None:
            idle_state[0] = 'neutral'
            idle_timer[0] = ct + 5000   # aspetta 5s dopo il comando prima del prossimo idle
            neutral_duration[0] = random.randint(8000, 18000)
        screen.fill(BGCOLOR)
