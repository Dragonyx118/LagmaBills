...
# ── SBuffi circolare ───────────────────────────────────────────────────
# deque(maxlen=N) scarta automaticamente il chunk più vecchio quando piena.
# Il lock protegge le operazioni composte (check + append), non la deque
# in sé (che è già thread-safe per operazioni singole grazie al GIL).
...
# ── Accumulatore audio 
MIN_SAMPLES = int(os.getenv("MIN_CHUNK_SAMPLES", str(16000 * 3)))  # default 3s
...
async def on_audio_ready(audio: np.ndarray):
    ...
    with _accum_lock:
        _accum.append(audio)
        total = sum(len(a) for a in _accum)
        if total < MIN_SAMPLES:
            logger.debug(f"Accumulato {total}/{MIN_SAMPLES} campioni — in attesa")
            return
        # Abbastanza audio: concatena e svuota fuori dal lock
        chunk  = np.concatenate(_accum)
        _accum = []

    # _ring_put fuori dal lock
    inserted = _ring_put(chunk)
    if inserted:
        logger.debug(f"Chunk {len(chunk)/16000:.2f}s in ring buffer ({len(_ring)}/{AUDIO_RING_SIZE})")
    else:
        logger.warning(
