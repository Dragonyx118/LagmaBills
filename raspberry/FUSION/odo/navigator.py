...
def control_loop():
    ...
    while True:
        ...
        if mqtt_client and mqtt_client.is_connected():
            # Fix: il firmware ESP32 si aspetta {"cmd":"mecanum","vx":...}
            # Senza il campo "cmd" il mqttCallback del firmware ignora il messaggio
            mecanum_cmd = {"cmd": "mecanum", "vx": cmd["vx"], "vy": cmd["vy"], "vr": cmd["vr"]}
            mqtt_client.publish(T_MOTORI, json.dumps(mecanum_cmd))
...
def on_message(client, userdata, msg):
    ...
    if topic == T_ODOMETRIA:
        ...
        if cmd == "start":
            ...
            with state_lock:
                state["running"] = False
            # Fix: formato mecanum con campo "cmd" richiesto dal firmware
            client.publish(T_MOTORI, json.dumps({"cmd": "mecanum", "vx": 0, "vy": 0, "vr": 0}))
...
def main():
    ...
    try:
        ...
        mqtt_client.publish(T_MOTORI, json.dumps({"cmd": "mecanum", "vx": 0, "vy": 0, "vr": 0}))
    finally:
