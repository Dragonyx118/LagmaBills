...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sensor-cell.ok      { border-color: var(--border); }  /* stato normale esplicito */
...
function updateSensors(sensors) {
  ...
  for (const name of ['FRONTE','RETRO','SINISTRA','DESTRA']) {
    ...
    if (!el) continue;
    // Bug fix: usa == (non ===) per confrontare con 9999 perché dal JSON
    // può arrivare come float 9999.0; val <= 0 copre anche letture nulle
    if (val == null || val == 9999 || val <= 0) {
      el.textContent = '---';
      cell.className = 'sensor-cell';   // rimosso 'ok' che non esiste nel CSS
      continue;
    }
    el.textContent = val.toFixed(0);
    // Bug fix: classe 'ok' non esiste nel CSS → usa stringa vuota per stato normale
    cell.className = 'sensor-cell' + (val < NEAR ? ' near' : val < MID ? ' mid' : '');
  }
}

function updateCliff(cliffData) {
  // Bug fix: supporta sia il payload di robot/cliff/stato
  //   {"cliff_f": bool, "cliff_r": bool, "cf_cm": float, "cr_cm": float}
  // che il sotto-oggetto cliff annidato nella mappa
  //   {"cliff_f": bool, "cliff_r": bool}  (senza cf_cm/cr_cm)
  const cf = cliffData.cliff_f || false;
  const cr = cliffData.cliff_r || false;

  // Distanze grezze: mostrale se disponibili, altrimenti solo OK/DIRUPO
  const cfCm = (cliffData.cf_cm != null && cliffData.cf_cm >= 0)
    ? cliffData.cf_cm.toFixed(0) + ' cm' : null;
  const crCm = (cliffData.cr_cm != null && cliffData.cr_cm >= 0)
    ? cliffData.cr_cm.toFixed(0) + ' cm' : null;

  document.getElementById('cliff-f-val').textContent =
    cf ? '⚠ DIRUPO' : (cfCm ? cfCm : 'OK');
  document.getElementById('cliff-r-val').textContent =
    cr ? '⚠ DIRUPO' : (crCm ? crCm : 'OK');
